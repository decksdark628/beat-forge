...
func export_song(s:Song):
	...
	for t in song.targets:
		var line:PackedStringArray = PackedStringArray([t.expected_input, t.tick])
...
func _extract_content(file:FileAccess):
	while file.get_position() < file.get_length():
		...
		if line[0].is_valid_int() and line[1].is_valid_int():
			song.add_target(int(line[0]), int(line[1]))
